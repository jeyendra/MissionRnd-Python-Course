...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert [ ] == result  #what happened here? error raised
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['all', 'any', 'enumerate', 'filter', 'frozenset', 'list', 'map', 'max', 'min', 'next', 'reversed', 'set', 'sorted', 'sum', 'tupple', 'zip']
    assert 16== len(funcs)
