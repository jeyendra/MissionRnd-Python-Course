...
def test_exception_basic():
    ...
    # what print statements do you see when you run the above code - in the "captured output"?"starting test" what is the stack trace?"sample assertion error"
...
def test_exception_flow_1():
    ...
    try:
        fruit = fruit.upper()
        result.append("one")
        fruit.missingmethod() # what happens to the control flow here?CONTROL GOES TO EXCEPT BLOCK
        result.append("two")  # will this line get executed? NO
...
def test_missing_except():
    ...
    try:
        result.append("one")
        #what happens now? raises exception fix it with an appropriate try except so that result is as expected.
        fruit.missingmethod()
    except:
        result.append("two")
